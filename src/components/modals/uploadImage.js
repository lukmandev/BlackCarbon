...
import { setActive, setMessage } from '../../redux/reducers/uploadImageModal';

const UploadImageModal = () => {
	const dispatch = useDispatch();
	const state = useSelector(allState => {
		return {
			...allState.uploadImageModal
		}
	});
	const { changeUserAvatar } = EditProfileService();

	const fileInput = useRef(null);
	const uploadImage = () => {
		if(fileInput.current.files.length){
			changeUserAvatar({avatar: fileInput.current.files[0]});
		}	
	}
